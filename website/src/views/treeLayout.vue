<template>
  <div>
    <header>树布局算法</header>
    <h2>引入</h2>
    <pre>import treeLayout from 'jsdoor/treeLayout/index.js';</pre>
    <h2>创建计算对象</h2>
    <p>
      此算法是用于计算一个树结构的坐标的方法，传递配置config就可以获取树对象实例：
    </p>
    <pre>var treeObj=treeLayout(config);</pre>
    <h3>配置数据结构</h3>
    <p>
    ...
    <pre>var resultData=treeObj(data);</pre>
    <h3>返回的数据结构</h3>
